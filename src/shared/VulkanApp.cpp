...
void VulkanApp::drawGTFInspector_Animations(GLTFIntrospective &intro) {
	...
	if (ImGui::Begin(
		...
		for (uint32_t a = 0; a < intro.animations.size(); ++a)
		{
			...
			if (state) {
				if (!oState) {
					uint32_t freeSlot = static_cast<uint32_t>(intro.activeAnim.size()) - 1;

					if (auto nf = std::find(intro.activeAnim.begin(), intro.activeAnim.end(), ~0u); nf != intro.activeAnim.end()) {
						freeSlot = static_cast<uint32_t>(std::distance(intro.activeAnim.begin(), nf));
					}
	...
	uint32_t numSamples, lvk::Format colorFormat)
{
	if (gridPipeline.empty() || pipelineSamples != numSamples) {
		gridVert = loadShaderModule(ctx_.get(), "../../src/shaders/grid/Grid.vert");
		gridFrag = loadShaderModule(ctx_.get(), "../../src/shaders/grid/Grid.frag");
